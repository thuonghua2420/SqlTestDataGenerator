/*
    EnterpriseRetailDemo.sql
    Creates a demo SQL Server database with 26 related tables.
    Every table receives exactly 100 rows.
    DBeaver note: run the full file with "Execute SQL Script", not single-statement execute.

    If you want a different database name, replace EnterpriseRetailDemo below
...
BEGIN
    THROW 51000, N'Database EnterpriseRetailDemo already exists. Rename it in this script or remove the existing database before rerunning.', 1;
END;
CREATE DATABASE EnterpriseRetailDemo;
USE EnterpriseRetailDemo;
SET NOCOUNT ON;
...
(
    n INT NOT NULL CONSTRAINT PK_Numbers PRIMARY KEY CLUSTERED
);
